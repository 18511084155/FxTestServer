...
public abstract class MultiLineReceiver implements IShellOutputReceiver {
    ...
    public final void appendOutput(byte[] data, int offset, int length) {
        if (!isCancelled()) {
            // ok we've got a string
            String value;
            try {
                value = new String(data, offset, length, "UTF-8"); //$NON-NLS-1$
            } catch (UnsupportedEncodingException e) {
                // normal encoding didn't work, try the default one
            ...
            if (!TextUtils.isEmpty(value)) {
                ...
                do {
                    ...
                    // and we store it to be processed for the next packet
                    if (index == -1) {
                        mUnfinishedLine = value.substring(start);
                        break;
                    }

                    // so we found a \r\n;
                    // extract the line
                    String line = value.substring(start, index);
                    if (mTrimLines) {
                        line = line.trim();
                    }
                    mArray.add(line);
